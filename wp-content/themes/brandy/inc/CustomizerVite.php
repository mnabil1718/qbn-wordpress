<?php
/**
 * Handle CustomizerVite admin app.
 *
 ...
 * Declare class
 */
class CustomizerVite {

	private static $root = 'src';

	private static $path_to_dist = '/assets/dist/customizer/';
	...
	public static function enqueue_vite( $script = 'main.tsx', $port = '3001' ) {
		...
		add_filter(
			...
			function ( $tag, $handle, $src ) {
				if ( str_contains( $handle, 'module/brandy/' ) ) {
					$str  = "type='module'";
		...
		add_filter(
			'script_loader_src',
			function( $src, $handle ) {
				if ( str_contains( $handle, 'module/brandy/vite' ) && strpos( $src, '?ver=' ) ) {
	...
	public static function register( $entry, $port ) {
		...
		$deps = array( 'react', 'react-dom', 'wp-block-editor', 'wp-core-data', 'wp-media-utils', 'wp-widgets' );

		if ( ! $url ) {
			return '';
		}
		if ( constant( 'BRANDY_IS_DEVELOPMENT' ) ) {
			wp_enqueue_script( 'module/brandy/vite', "http://localhost:$port/@vite/client", $deps, 1.0, false );
		}
		wp_register_script( "module/brandy/$entry", $url, $deps, true, true );
		wp_enqueue_script( "module/brandy/$entry" );
	}
	...
	private static function css_tag( $entry ) {
		...
		foreach ( self::css_urls( $entry ) as $key => $url ) {
			wp_register_style( "brandy/$key", $url, array(), 1.0 );
			wp_enqueue_style( "brandy/$key", $url, array(), 1.0 );
		}
		return $tags;
	}
